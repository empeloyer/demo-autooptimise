    ...
    fun start(context: Context) {
        ...
        job = scope.launch {
            try {
                ...
                val aiEngine = AppContainer.aiInferenceEngine(appContext)
                val aiConfig = if (settings.aiAssistEnabled && aiEngine.isLoaded) {
                    AiAssistConfig(aiEngine, settings.aiMode, settings.aiConfidenceThreshold, settings.aiOverrideThreshold)
                } else null
                val summary = engine.run(db, periodDays, aiConfig = aiConfig) { p -> progress.value = p }
